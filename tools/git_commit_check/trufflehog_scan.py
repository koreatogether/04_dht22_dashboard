...
class DHT22ScanResult:
    ...
    def (
     ...
     "count > 0:
        ...
        self.data["summary"]["critical_findings"] += self._count_critical_findings(
            findings
        )
        self.data["summary"]["privacy_findings"] += self._count_privacy_findings(
            findings
        )
        self.data["summary"]["(
    ...
    de" +
     ...
     "TimeoutExpired:
            return {
                ...
                ")ok": False,
                "stdout": "",
                "stderr": f"timeout after {self.timeout
...
def ensure_trufflehog() -> bool:
    ...
    if platform.system() == "Windows":
        print(
            c(
                "   1. https://github.com/trufflesecurity/trufflehog/releases 에서 다운로드",
                "36",
            )
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--fail-on-find", action="store_true", help="민감정보 발견 시 실패 (CI/CD용)"
    )
    parser.add_argument("--timeout", type=int, default=300, help="스캔 최대 시간(초)")
    parser.add_argument(
        "--exclude-patterns",
        nargs="*",
        default=DHT22_EXCLUDE_PATTERNS,
        help="제외할 파일 패턴",
    )
    parser.add_argument(
        "--privacy-mode", action="store_true", help="개인정보 보호 강화 모드"
    )
