...
class AutoTestRunner:
    ...
    def run_all_quality_checks(self) -> bool:
        ...
        checks = [
            ...
            ("MyPy 타입 검사", ["python", "-m", "mypy", "src/", "--ignore-missing-imports"], "mypy"),
    ...
    def run_dht22_functional_tests(self) -> bool:
        """DHT22 기능 테스트 실행"""
        print("🌡️ DHT22 기능 테스트 실행 중...")
        
        functional_tests = [
            ("DHT22 시뮬레이터 테스트", self._test_dht22_simulator),
            ("환경 계산 함수 테스트", self._test_environmental_calculations),
            ("WebSocket 연결 테스트", self._test_websocket_connection),
            ("API 엔드포인트 테스트", self._test_api_endpoints),
            ("데이터 유효성 테스트", self._test_data_validation)
        ]
        ...
        for name, test_func in functional_tests:
            ...
            try:
                success = test_func()
                results[name] = {"success": success, "timestamp": datetime.now().isoformat()}
    ...
    def continuous_monitoring(self, interval: int = 30, max_iterations: int = 10):
        """지속적 품질 모니터링"""
        print(f"🔄 {interval}초 간격으로 품질 모니터링 시작 (최대 {max_iterations}회)...")
        ...
        while iteration < max_iterations:
            try:
                ...
                # 결과 리포트 생성
                self.generate_monitoring_report(iteration)
    ...
    def generate_test_report(self) -> str:
        ...
        passed_quality = sum(1 for result in self.quality_results.values() 
                           if isinstance(result, dict) and result.get("success", False))
...
"""
    ...
    def _create_sample_test_file(self, phase_num: int):
        """샘플 테스트 파일 생성"""
...
'''
    ...
    def _test_dht22_simulator(self) -> bool:
        ...
        try:
            ...
            from dht22_main import DHT22Simulator, calculate_heat_index, calculate_dew_point
            ...
            # 데이터 유효성 검사
            required_fields = ["timestamp", "temperature", "humidity", "heat_index", "dew_point"]
    ...
    def _test_environmental_calculations(self) -> bool:
        """환경 계산 함수 테스트"""
        try:
            sys.path.insert(0, str(self.project_root / "src" / "python" / "backend"))
            from dht22_main import calculate_heat_index, calculate_dew_point
    ...
    def _save_quality_results(self):
        """품질 검사 결과 저장"""
        results_file = self.results_dir / f"quality_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_failure_recommendations(self) -> str:
        ...
        for check_name, result in self.quality_results.items():
            if isinstance(result, dict) and not result.get("success", False):
                if check_name == "ruff":
                    recommendations.append("- Ruff 린트 오류: `ruff check --fix src/` 실행")
                    ...
                    recommendations.append(f"- {check_name}: 관련 도구 설정 및 코드 수정")
        
        return "\n".join(recommendations) if recommendations else "모든 검사가 통과했습니다! 🎉"
    
    def generate_monitoring_report(self, iteration: int):
        """모니터링 리포트 생성"""
...
def main():
    ...
    if args.phase:
        ...
        print("사용법: python auto_test_runner.py [--phase N] [--quality] [--functional] [--monitor] [--all] [--report]")
        return
