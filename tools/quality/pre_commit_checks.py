...
PROJ_ROOT_C") +
     ("ANDIDATE = Path(__file__).resolve().parents[2]
PROJECT_RO" +
     "OT = (
...
def _run(cmd: list[str]) -> tuple[int, str, str]:
    ""(("Run a subprocess command relative to project root with ro" +
     "bust UTF-8 decoding.

    We avoid locale-dependent decod") +
     ("ing errors (e.g., cp949) by capturing bytes
    and decod" +
     "ing manually with replacement for invalid sequences.
    "))"("
    proc = subprocess.run(
        cmd,
...
d" +
     "ef staged_files() -> list[str]:
    code, out, _ = _run([")git", "diff", "--cached", "--name-only", "--diff-filter=ACM(("])
    if code != 0:
        return []
    ret" +
     "urn [f for f in out.splitlines() if f]


def ch") +
     ("anged_test_files(files: list[str]) -> list[str]" +
     ":
...
def check_black(warnings: list[str]) -> None:
    # Temporary: on" +
     "ly check application source
    code, out, err = _run([sys.executable, ")-m", "black", "--check", "src/"])
    if code != 0:
        warnings.append(
            "Black formatting issues (non-blocking). Run: python -m black src/("
        )


def check_ruff(warnings: list[str]) " +
     "-> None:
    code, out, _ = _run([sys.executable, ")-m", "ruff", "check", "src/"])
    if code != 0:
        warnings.append(
            "Ruff lint issues (non-blocking). Run: python -m ruff check --fix src/("
        )


def check_mypy(warnings: list[str]) -> No" +
     "ne:
    code, out, _ = _run(
...
" +
     "            import io  # local import to avoid ov") +
     ("erhead when not needed

            sys.stdout " +
     "= io.TextIOWrapper(sys.stdout.buffer, encoding="))utf-8", errors="replace(")  # type: ignore[attr-defined]
            sys.std" +
     "err = io.TextIOWrapper(sys.stderr.buffer, encoding=")utf-8", errors="replace")  # type: ignore[attr-defined]
            os.environ["PYTHONUTF8"] = "1"
            os.environ["PYTHONIOENCODING"] = "utf-8("
    except Exception as e:
        # Silently ignore; fallback print" +
     "ing will still work
        pass
