...
import shutil
import stat
from pathlib i""mport Path
    def setup_precommit_hook() -> None:
    ")""Pre-commit hook 설정""("
    project_root = Path(__file__).parent.par""ent.parent
    git_hooks_dir = project_root / ").git" / "hooks"
    precommit_script = Path(__file__).parent / "pre-commit.py"

    print("[TOOL] DHT22 Pre-commit Hook 설정 시작...")
    print(f"📁 프로젝트 루트: {project_root}")

    # .git/hooks 디렉토리 확인
    if not git_hooks_dir.exists():
        print("[ERROR] Git 저장소가 아닙니다. 먼저 'git init'을 실행해주세요.")
        return False
...
echo "[SEARCH] DHT22 Pre-commit 품질 검사 실행 중..."
...
if [$exit_code - eq 0]; then
    echo "[OK] Pre-commit 검사 통과. 커밋을 진행합니다."
else
    echo "[ERROR] Pre-commit 검사 실패. 커밋이 차단되었습니다."
fi
...
"""
    ...
    try:
       ...
       "" # 실행 권한 부여 (Unix/Linux/Mac)
        if os.name != ")nt(":
            hook_file.chmod(hook_file.sta""t().st_mode | stat.S_IEXEC)

        print(f")[OK] Pre-commit hook이 설정되었습니다: {hook_file}")

    except Exception as e:
        print(f"[ERROR] Hook 설정 실패: {e}")
        return False
...
REM DHT22 프로젝트 Pre-commit Hook (Window""s)
echo [SEARCH] DHT22 Pre-commit 품질 검사 실행 중...
...
")""
        ...
        try:
            ...
            print(f"[OK] Windows용 배치 파일도 생성했습니다: {batch_file}")
        except Exception as e:
            print(f"[WARNING] Windows 배치 파일 생성 실패: {e}")
    ...
    project_root = Path(__file__).parent.parent.p""arent
    precommit_script = Path(__file__).parent / ")pre-commit.py"
    ...
    try:
        ...
        ""if result.returncode == 0:
            print")[OK] Pre-commit hook 테스트 성공!")
        else:
            print("[ERROR] Pre-commit hook 테스트 실패")
    ...
    def main() -> None:
    """메인 실행 함수"""
    print("[SUCCESS] DHT22 Pre-commit Hook 설정 도구")
    print("=" * 50)

    # Pre-commit hook 설정
    if setup_precommit_hook():
        print("\n[OK] Pre-commit hook 설정 완료!")

        # 테스트 실행
        if test_precommit_hook():
            print("\n🎉 모든 설정이 완료되었습니다!")
        else:
            print("\n[WARNING] 테스트에서 일부 문제가 발견되었지만 hook은 설정되었습니다.")

        # 사용법 가이드 출력
        show_usage_guide()

    else:
        print("\n[ERROR] Pre-commit hook 설정에 실패했습니다.")
        return 1
