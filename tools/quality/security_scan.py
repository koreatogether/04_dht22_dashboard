...
class SecurityScanner:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.security_patterns = {
            "hardcoded_secrets": [
                ...
                (r'secret_key\s*=\s*["\'][^"\']+["\']', "하드코딩된 시크릿 키"),
                (r'token\s*=\s*["\'][^"\']+["\']', "하드코딩된 토큰"),
                (r'["\'][A-Za-z0-9]{32,}["\']', "의심스러운 긴 문자열 (API 키 가능성)"),
            ],
            "sql_injection": [
                (r'execute\s*\(\s*["\'].*%.*["\']', "SQL 인젝션 취약점 (문자열 포맷팅)"),
                (r'query\s*=\s*["\'].*\+.*["\']', "SQL 인젝션 취약점 (문자열 연결)"),
                (r'SELECT.*\+.*FROM', "SQL 인젝션 취약점 (동적 쿼리)"),
            ],
            "command_injection": [
                (r'os\.system\s*\(.*\+', "명령어 인젝션 취약점"),
                (r'subprocess\..*shell=True', "셸 인젝션 위험"),
                (r'eval\s*\(', "코드 인젝션 위험 (eval 사용)"),
                (r'exec\s*\(', "코드 인젝션 위험 (exec 사용)"),
            ],
            "file_operations": [
            ...
            "network_security": [
                (r'requests\.get\s*\(.*verify=False', "SSL 인증서 검증 비활성화"),
                (r'urllib.*verify=False', "SSL 인증서 검증 비활성화"),
                (r'http://(?!localhost|127\.0\.0\.1)', "HTTP 사용 (HTTPS 권장)"),
            ]
        }
    ...
    def _scan_config_files(self):
        ...
        config_patterns = [
            "*.env", "*.ini", "*.conf", "*.yaml", "*.yml", 
            "*.json", "*.toml", "requirements*.txt"
        ]
    ...
    def _scan_config_content(self, file_path: Path, content: str):
        """설정 파일 내용 스캔"""
        lines = content.split('\n')
    ...
    def _check_vulnerable_packages(self, file_path: Path, content: str):
        """알려진 취약한 패키지 검사"""
        # 알려진 취약한 패키지 목록 (예시)
        vulnerable_packages = {
            "django": ["<3.2.13", "보안 업데이트 필요"],
            "flask": ["<2.0.3", "보안 업데이트 필요"],
            "requests": ["<2.25.1", "보안 업데이트 필요"],
        }
                    ...
                    if os.name == 'nt':
                        ...
                        if mode != '600':  # 소유자만 읽기/쓰기
                            self._add_warning({
                                "file": str(file_path.relative_to(self.project_root)),
                                "category": "file_permissions",
                                "description": f"부적절한 파일 권한: {mode} (권장: 600)",
                                "severity": "medium"
                            })
    ...
    def _save_results(self):
        ...
        results_file = results_dir / f"security_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def print_results(self):
        ...
        if self.scan_results["vulnerabilities"]:
            ...
            for vuln in self.scan_results["vulnerabilities"][:5]:  # 최대 5개만 출력
                print(f"   - {vuln['file']}:{vuln.get('line', '?')} - {vuln['description']}")
        ...
        if self.scan_results["warnings"]:
            ...
            for warning in self.scan_results["warnings"][:3]:  # 최대 3개만 출력
                print(f"   - {warning['file']}:{warning.get('line', '?')} - {warning['description']}")
