...
def check_ruff(warnings: list[str]) -> None:
    code, out, _ = _run([sys.executable, "-m", "ruff", "check", "src/", "tools/"])
    if code != 0:
        warnings.append(
...
def check_docs(files: list[str], warnings: list[str]) -> None:
    if any(f.endswith(".py") and not f.startswith("docs/") for f in files):
        if not any(f.startswith("docs/") or f == "README.MD" for f in files):
            warnings.append(
                "Code changed but no docs updated (docs/ or README.MD). "
                "Consider updating release notes."
            )
...
def main() -> int:
    _force_utf8_stdio()
    _e("[SEARCH] Running fast pre-commit checks (clean script)...")
    errors: list[str] = []
    warnings: list[str] = []
    ...
    _e("\n================ Summary ================")
    if errors:
        _e(f"[ERROR] Errors ({len(errors)}):")
        for e in errors:
            print(f"  - {e.splitlines()[0]}")
    else:
        _e("[OK] No blocking errors.")
    ...
    if errors:
        _e("\n[ERROR] Commit blocked. Fix errors above.")
        return 1

    _e("\n[OK] Pre-commit checks passed. Commit allowed.")
    return 0
