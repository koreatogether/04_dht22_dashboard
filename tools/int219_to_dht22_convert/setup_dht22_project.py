...
def convert_files(target):
    ...
    for file_path in target.rglob("*.py"):
        if convert_file_content(file_path, conversions):
            converted_count += 1
    
    print(f"  ✅ {converted_count}개 Python 파일 변환 완료")

def convert_file_content(file_path, conversions):
    """단일 파일 내용 변환"""
    try:
...
"""
    ...
    requirements_dev_content = """# 개발 의존성
pytest>=7.0.0
pytest-cov>=4.0.0
...
"""
    ...
    print("  ✅ requirements-dev.txt 생성 완료")

def create_dht22_specific_files(target):
    """DHT22 특화 파일들 생성"""
...
def calculate_comfort_index(temperature_c: float, humidity: float) -> dict:
    ...
    """
    # 불쾌지수 계산
    discomfort_index = 0.81 * temperature_c + 0.01 * humidity * (0.99 * temperature_c - 14.3) + 46.3
    ...
    return {
        "index": round(discomfort_index, 1),
        "level": comfort_level,
        "color": color
    }

def get_environmental_status(temperature_c: float, humidity: float) -> dict:
    """
    종합 환경 상태 평가
    """
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("3. .venv\\Scripts\\activate (Windows) 또는 source .venv/bin/activate (Linux/Mac)")
        print("4. pip install -r requirements-dev.txt")
    else:
        print("❌ 프로젝트 초기화에 실패했습니다.")