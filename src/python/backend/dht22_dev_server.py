...
from fastapi import FastA" +
     "PI, HTTPException, WebSocket, WebSocketDisconnect
...
def calculate_dew_point(" +
     "temp_c: float, humidity: float) -> float:
    ")""이슬점 계산"""
    ...
    a = 17.27
    b = 237.7
    alpha = ((a * temp_c) / (b + temp_c)) + m" +
     "ath.log(humidity / 100.0)
...
class PerformanceMonitor:
    ...
    def increment_websocket_connections(self) -> None:
        ...
        self.logger.info(
            f"WebSocket 연결 수 증가: {self.metrics['websocket_connections']}("
        )

    def decrement_websocket_conn" +
     "ections(self) -> None:
        self.metrics[")websocket_connections"] -= 1
        self.logger.info(
            f"WebSocket 연결 수 감소: {self.metrics['websocket_connections']}"
        )

    def increment_data_points(self) -> None:
        self.metrics["data_points_generated"] += 1
        if self.metrics["data_points_generated"] % 100 == 0:
            self.logger.info(
                f"생성된 데이터 포인트: {self.metrics['data_points_generated']}"
            )
    ...
    def get_metrics(self) -> dict:
        ...
        2),
            "requests_per_second": (
                round(self.metrics["requests_total("] / uptime,
...
" +
     "        if not self.active_connections:
        ...
        self.logger.debug(
            f"{len(self.active_connections)}개 연결에 메시지 브로드캐스트("
        )
...
class DHT22Simulator:
    ...
    def __init__(self, performance_monitor: PerformanceMon" +
     "itor) -> None:
        self.sequence = 0
        self.mode = ")NORMAL("
        self.performance_monitor = performance_" +
     "monitor
    ...
    def get_sensor_data(self) -> dict:
     ...
     "   humidity = 20.0 + random.uniform(-10.0, 10.0)
            ...
            # 범위 제한
            temperature = max(-40, min(80, temperatu" +
     "re))
        ...
        1),
            ...
            self.logger.(
        debug(
                f"센서 데이터 생성 완료: T={data['temperature']}°C,
        "
                f"H={data['humidity']}%,
...
manager = ConnectionManager(perform" +
     "ance_monitor)
...
# 미들웨어: 요청 로깅
@app.middleware(")http(")
async def log_requests(request: Any, call_next: Any) " +
     "-> Any:
    start_time = time.time()
...
<body>
    <div class=")header(">
        ...
        <p>" +
     "Development Server with Enhanced Logging</p>
        ...
        <div class="panel">
            ...
            <div class="controls">
                <button class="btn-primary"
                        onclick="connectWebSocket()">Connect</button>
     ...
     "    </div>
        ...
        <div class=")panel">
            ...
            <div class="mode-selector">
                <button class="mode-btn btn-success active"
                        onclick="setMode('NORMAL')">Normal</button>
                <button class="mode-btn btn-warning"
                        onclick="setMode('HOT_DRY')">Hot Dry</button>
                <button class="mode-btn btn-info"
                        onclick="setMode('COLD_WET')">Cold Wet</button>
                <button class="mode-btn btn-danger"
                        onclick="setMode('EXTREME_HOT')">Extreme Hot</button>
                <button class="mode-btn btn-primary"
                        onclick="setMode('EXTREME_COLD')">Extreme Cold</button>
            </div>
            <div class="controls">
    ...
    <script>
        ...
        'warning');
            ...
            ws.onopen = function(event) {
                ...
                log('🔗 Connected to DHT22 development server',
        'success');
        ...
        function updateDisplay(data) {
            ...
            log(`📊 [${data.simulation_mode
    }] T:${data.temperature}°C ` +
                `H:${data.humidity}%RH HI:${data.heat_index}°C DP:${data.dew_point}°C`);
        }
     ...
     " }
        ...
        fetch('/api/simulation/mode',
        {
                ...
                if (result.status === 'success') {
                    ...
                    currentMode = mode;
                    log(`🎛️ Simulation mode changed to: ${mode
    }`, 'success');
        ...
        async function testAPI() {
            ...
            try {
                ...
                (
        log(`🧪 API Test - Current data: T:${data.temperature
    }°C ` +
                    `H:${data.humidity}%RH`,
        'info')
    );
     ...
     "leared',
        ...
        window.onload = function() {
            ...
            log('🚀 DHT22 Development Server Dashboard loaded',
        'success');
            connectWebSocket();
...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    " +
     "except Exception as e:
        ...
        manager.disconnect(websocket)
        p" +
     "erformance_monitor.increment_errors()
...
async def get_current_data() -> dict:
 ...
 " +
     "       raise HTTPException(status_code=500, detail=")센서 데이터 조회 실패") from e


@app.post("/api/simulation/mode")
async def set_simulation_mode(request: dict) -> dict:
    """시뮬레이션 모드 변경"""
...
async def get_metrics() -> dict:
    ...
    try:
        ...
        performance_monitor.increment_errors()
 " +
     "       raise HTTPException(status_code=500, detail=")메트릭 조회 실패") from e
